...
function arrowFunctionExpression (node: Node) {
  ...
  return [
    ...
    ['div', // Used for flex layout
      ...
      ['.body', ...ensureNodeArray(node.body).map(x => walkTree(x))],
    ],
  ]
}
...
function catchClause (node: Node) {
  return [
    ...
    ['span.body', ...ensureNodeArray(node.body).map(x => walkTree(x))],
  ]
}
...
function classBody (node: Node) {
  return [
    'div.classBody',
    ...ensureNodeArray(node.body).map(x => walkTree(x)),
  ]
}
...
function classDeclaration (node: Node) {
  return [
    ...
    ['div.body', ...ensureNodeArray(node.body).map(x => walkTree(x))],
  ]
}
...
function functionDeclaration (node: Node): mixed[] {
...
function walkTree (node?: Node, fileData?: FileData): mixed[] {
  ...
  if (node.type === 'Program') {
    ...
    const bodyElements = ensureNodeArray(node.body).map(x => walkTree(x))
